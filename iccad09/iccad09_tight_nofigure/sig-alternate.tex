...
We check this assumption with SAT solver and find out proper value of these parameters.
...
or else it is called \textbf{unsatisfiable formula}.
...
Formally,
assume $\boldsymbol{F}$ is the formula over boolean variables set $V$,
...
Thus, we can merged $A|_{V-\{v\}}|^{v\gets 0}$ and $A|_{V-\{v\}}|^{v\gets 1}$ to remove $v$ from $A$,
...
%transition relation between states $T_{¨ª}\subseteq S\times S$,
...
Our algorithm cares about the input and output sequence of circuits $E$,
...
Thus,
$f^{-1}:O^l\to I$ is now a boolean function that takes finite length sequence $<o_{n+d-l},\dots , o_{n+d-1} >$ as input,
and computes $i_n$.
...
The third line constraints their output sequences $<o_{n+d-l},\dots , o_{n+d-1} >$ and $<o'_{n+d-l},\dots , o'_{n+d-1} >$ to be the same,
...
So can we find a tradeoff between these two extremes?
...
So now, in addition to parameter $d$ and $l$,
...
But this naive approach suffers from state space explosion problem.
...
There exists many much more efficient approaches
...
\begin{enumerate}\addtolength{\itemsep}{-0.5\baselineskip}
{\setlength{\baselineskip}{0.5\baselineskip}
\item assume $\boldsymbol{I_{var}}$ and $\boldsymbol{O_{var}}$ are respectively boolean variables set that represent $i_n$ and $<o_{n+d-l},\dots , o_{n+d-1} >$.
...
They will help to speedup the process of enumerating satisfying assignments of $F_E\wedge v\equiv 1$ on line 5.
...
and build a function $f^{-1}_v$ for it at \textbf{line 14},
...
\begin{enumerate}\addtolength{\itemsep}{-0.5\baselineskip}
{\setlength{\baselineskip}{0.5\baselineskip}
\item Assume $\boldsymbol{O_{var}}$ is boolean variables set that represents $<o_{n+d-l},\dots , o_{n+d-1} >$
...
BFL can't minimize it's satisfying assignments.
...
With this $A_{x_1}$,
existence of $v'\gets v_1\oplus v_2$ can be decided by checking unsatisfiability of following formula :
...
and $<o_{n+d-l},\dots , o_{n+d-1} >$ is represented by boolean variables set $O_{var}$.
...
Orna Grumberg et al.\cite{MEMEFFALLSAT} separates the variables set into important subset and non-important subset.
...
which starts from an empty set of assignments,
...
So many researchers propose synthesis algorithms that target XOR-intensive circuit.
...
These works normally express circuits with most general ESOP (exclusive sum of product) expressions.
