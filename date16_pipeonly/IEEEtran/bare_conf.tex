...
generate pipelined decoder with significantly improved timing.
...
\begin{figure}[b]
\begin{center}
\includegraphics[width=0.3\textwidth]{mealy}
\end{center}
...
\subsection{The halting algorithm that determines whether $i\in\vec{i}$ can be uniquely determined by a bounded sequence of $\vec{o}$}\label{subsec_chkextdec}

\begin{algorithm}[t]
\SetAlgoVlined
\KwIn{The input variable $i\in\vec{i}$.}
\KwOut{the answer of whether $i$ can be uniquely determined, and the maximal $p$, $l$ and $r$ reached.}
$p$:= 0 ;~$l$:= 0 ;~$r$:= 0 \;
...
Shen et al. \cite{ShenTCAD11} proposed Algorithm \ref{alg_fofc}
...
This approach is sound because when $F_{PC}(p,l,r)$ is unsatisfiable for a particular $p$. $l$ and $r$,
$i\in\vec{i}$ can always be uniquely determined by $<\vec{o}_p,\dots,\vec{o}_{p+l+r}>$.
...
This means $i\in\vec{i}$ can never be uniquely determined by a bounded sequence of $\vec{o}$.

Thus,
Algorithm \ref{alg_fofc} is a halting one because 
...
\end{center}
\caption{The encoder's general structure with pipeline}
  \label{fig_pipeenc}
\end{figure}
...
where $0\le j \le n-1$.
Obviously the set of all state variables in pipeline stages are a subset of $\vec{s}$,
that is:

\begin{equation}
\vec{s} \supseteq \bigcup _{0\le j \le n-1} \vec{stg}_j
\end{equation}
...
So,
as shown in Fig. \ref{fig_Tparalleldecomp},
the transition function $T$ can be seen as a parallel composition $C^0|C^1\dots|C^{n-1}|C^n$.
...
%  calling Algorithm \ref{alg_fofc} to find out $(p,l,r)$.
 ...
 identifying all state variables in each pipeline stage $\vec{stg}^j$. 
 \item In Section \ref{sec_char}, 
 characterizing $(C^j)^{-1}$,
 the inverse functions of $C^j$ shown in Fig. \ref{fig_pipeenc} ,
 to recover each pipeline stages $\vec{stg}^j$
 and input vector $\vec{i}$ from the next pipeline stage $\vec{stg}^{j+1}$ or the output vector $\vec{o}$.
\end{enumerate}

\section{Identifying pipeline stages}\label{sec_pipeinfer}

\subsection{Finding out $p$,$l$ and $r$}


We first call Algorithm \ref{uniqt1} for each $i\in\vec{i}$.
If there exists an $i$ that make Algorithm \ref{uniqt1} to return $(false,0,0,0)$,
then their must be a bug in the encoder that prevents $i$ from being recovered from $\vec{o}$.

Otherwise,
assume that Algorithm \ref{uniqt1} returns $p_i$, $l_i$ and $r_i$ for $i\in\vec{i}$,
we set the final $p$, $l$ and $r$ to the largest ones returned from Algorithm \ref{uniqt1}:

\begin{IEEEeqnarray*}{rCl}\label{eqn_plr}
  p:=max_{i\in\vec{i}}\{p_i\}\\
  l:=max_{i\in\vec{i}}\{l_i\}\IEEEyesnumber\\
  r:=max_{i\in\vec{i}}\{r_i\}
\end{IEEEeqnarray*}

Obviously,
for every $i\in\vec{i}$,
the clause set of $F_{PC}(p,l,r)$ is a super set of $F_{PC}(p_i,l_i,r_i)$.
So $F_{PC}(p,l,r)$ is also unsatisfiable for $i\in\vec{i}$.
This means all $i\in\vec{i}$ can be uniquely determined by $<\vec{o}_p,\dots,\vec{o}_{p+l+r}>$.
...
\For{$r':=r \to 0$} {
\ShowLnLabel{testr_1}
  \If{$r'\equiv 0$ or $F'_{PC}(p,l,r'-1)$ is satisfiable for some $i\in \vec{i}$} {
    break
  }
}
return $r'$
...
In Line \ref{testr_1},
if $F'_{PC}(p,l,r'-1)$ is satisfiable for $i\in\vec{i}$,
then $r'$ is the last one that makes $F'_{PC}(p,l,r')$ unsatisfiable,
...
Now,
with $p$ and $l$ defined in Equation (\ref{eqn_plr}) and $r'$ founded by Algorithm \ref{algo_remove2},
...
every state variable $s\in \vec{stg}^{r'}$ at the $p+l+r'$-th step can be uniquely determined by $\vec{o}_{p+l+r'}$.
That is,
...
that recovers $s\in \vec{stg}^{r'}$ from $\vec{o}$.
...
$\vec{stg}^j_{p+l+j}$ can be uniquely determined by $\vec{stg}^{j+1}_{p+l+j+1}$.
So for every $s\in\vec{stg}^j$,
...
According to Fig. \ref{fig_encexp},
$\vec{i}_{p+l}$ can be uniquely determined by the 1st pipeline stage $\vec{stg}^0_{p+l}$.
So for every input $i\in\vec{i}$,
...
can be used as the decoder's Boolean function that recovers $i\in\vec{i}$ from $\vec{stg}^0$.
...
%   automatically,'' in  ICCAD '09, pp. 381--388,2009. 
% 
% \bibitem{ShenTCAD10}
% S.~Shen, Y.~Qin, K.~Wang, L.~Xiao, J.~Zhang, and S.~Li., ``Synthesizing
%   complementary circuits automatically,'' \emph{IEEE Transactions on
%   Computer-Aided Design of Integrated Circuits and Systems}, vol.~29, no.~8,
%   pp. 29:1191--29:1202, August 2010. 
% 
%   \bibitem{ShenTCAD11}
...
%   pp. 30:1556--30:1563,2011. 
% 
% \bibitem{ShenICCAD11}
% S.~Shen, Y.~Qin, and J.~Zhang, ``Inferring assertion for complementary
%   synthesis,'' in ICCAD'11, pp. 404--411, 2011. 
% 
% \bibitem{ShenTCAD12}
...
%   \url{https://github.com/shengyushen/todaes/blob/master/todaes.pdf}.
% 
% \bibitem{InterpBoolFunction}
% J.~R. Jiang, H.~Lin, and W.~Hung, ``Interpolating functions from large boolean
%   relations,'' in ICCAD'09, pp. 779--784,2009.
% 
% 
% \bibitem{Craig}
...
% 
% \bibitem{EXTSAT}
% N.~E{\'{e}}n and N.~S{\"{o}}rensson, ``An extensible sat-solver,'' in SAT'03., pp. 502--518,2003.
% 
% \bibitem{pcie}
...
% 
% \bibitem{scalableFuncDep}
% C.~Lee, J.~R. Jiang, C.~Huang, and A.~Mishchenko, ``Scalable exploration of
%   functional dependency by interpolation and incremental SAT solving,'' in ICCAD'07,
%   pp. 227--233,2007.
% 
%   
% \end{thebibliography}
