...
在通讯和多媒体芯片设计项目中，
最困难的一个工作是针对特定的协议，
如以太网\upcite{IEEE8023_S4} 和PCI Express \upcite{pcie21}，
设计相应的编码器和解码器.
其中编码器负责将输入$\vec{i}$ 映射至输出$\vec{o}$，
而解码器则负责从$\vec{o}$中恢复$\vec{i}$。
对偶综合\upcite{ShenICCAD09,ShenTCAD10,ShenTCAD11,ShenICCAD11,ShenTCAD12,LiuICCAD11,LiuTCAD12,TuDAC13}
假设$\vec{i}$ 总能够被$\vec{o}$的一个有限序列唯一决定，
以自动产生相应的解码器。
其中，
解码器的布尔函数可以使用Jiang et al. \upcite{InterpBoolFunction}提出的算法，
该算法基于Craig插值\upcite{Craig}。
...
在复杂的工业界实际编码器，
如PCI Express \upcite{pcie21} 和以太网\upcite{IEEE8023_S4}，
上的实验表明，
