...
为了得到一个停机算法，
我们需要区分上述两种情形的手段。
国防科技大学的沈胜宇\upcite{ShenTCAD11}和Liu et al.\upcite{LiuICCAD11}分别独立提出了类似的全新的解决方案。
该方案如图\ref{fig_ln}所示，
该图类似于图\ref{fig_pc}，
...
沈胜宇\upcite{ShenTCAD12}和Liu et al.\upcite{LiuICCAD11} 独立的发现可以通过Craig插值加速解码器的生成。
该算法还将作为下一章的一部分单独介绍，
因此不在这里展开描述。

沈胜宇\upcite{ShenTCAD12} 自动的发掘能够使得解码器存在的前提条件。
并首次发现在同一个前提条件下，
存在多个不同的解码器。
为了方便用户区分和选择，
...
现代复杂通讯协议的编码器中，广泛采用了流水线和流控机制\cite{flowcontrol}技术。
以提高性能并增强对环境的适应能力。
