...
而另一方面，
现代复杂通讯协议的编码器中，广泛采用了流控\cite{flowcontrol}和流水线等复杂内部结构，
...
widely employ flow control\cite{flowcontrol} and pipeline mechanism to boost performance and their ability of adopting to complex application environment.
...
$\vec{d}$ can be uniquely determined by $\vec{o}$ only when $valid(\vec{f})\eq true$.
...
First,
it infers the flow control predicate on inputs with Qin et al. \cite{QinTODAES15}'s algorithm.
Second,
...
Finally,
the decoder's Boolean functions that recover each pipeline stage and input are characterized with Jiang et al. \cite{InterpBoolFunction}'s algorithm based on Craig interpolant.
