...
Such technologies that enumerate all satisfying assignments of a formula are called \textbf{ALLSAT}.
...
transition relation between states $T_{ï¿½ï¿½}\subseteq S\times S$,
...
which make it impossible to characterize $f^{-1}$ for large $m$.
...
We solve this problem by discovering XOR gates with $\boldsymbol{XORMIN}$ function.
\item There are lots of redundant clauses in $F_E$.
We remove these clauses with by extracting unsatisfiable core with $\boldsymbol{BFL\_UNSAT}$ function.
\end{enumerate}

So,
we present below our new algorithm to characterize $f^{-1}$.
Its structure is very similar to algorithm 3,
with our improvements presented in boldface.
...
This clause set must be a subset of $F_E$,
and in most case,
it is much smaller than $F_E$,
as shown in experimental result.

So we propose the function $\boldsymbol{SIMPLIFY(F_E,v)}$ to simplify $F_E$ for every particular $v$:
\begin{enumerate}
\item Extracting unsatisfiable core $F_E^{UNSAT}$ from following formula (\ref{checkUA_v}) with depth first approach in Lintao Zhang et al.\cite{VALIDSAT}:
...
\item Intersecting the clause set of $F_E$ and $F_E^{UNSAT}$ to get formula $F_E^v$
...
Because $I_{var}$ is the boolean variables set that represent input alphabet $i_n$,
...
we must make sure that $F_E\wedge v$ and $F_E^v\wedge v$ have the same set of satisfying assignment on the variables set $O_{var}$,
which will be enumerated by algorithm 3.
...
According to definition of $F_E$ in formula (\ref{unfolding}) and (\ref{checkUA}),
...
This is contrary to formula (\ref{checkUA}),
so value on $v$ can't be 0.
\end{enumerate}
\end{IEEEproof}

So now,
we can be sure that replacing $F_E$ with $F_E^v$ to characterize $f_{-1}^v$ is safe.
And we will also show in experimental result that such replacing will significantly reduce $F_E$ size and run time overhead.

\subsection{Minimizing Satisfying Assignments by Extracting Unsatisfiable Core}\label{subsec_bfl}

In algorithm 2 line 4,
...
According to implementation of $BFL$ in line 8 of algorithm 1,
every $u\in U$ is tested one by one,
and if the formula in line 9 is unsatisfiable,
...
\begin{enumerate}
\item Computing unsatisfiable core $F^{US}$ of $F\wedge \neg v\wedge A|_{U -\{u\}}$ with depth first approach in Lintao Zhang et al.\cite{VALIDSAT}:
\item Computing new $A$ by intersecting clause set of $A|_{U-\{u\}}$ and $F^{US}$
\end{enumerate}
...
\item \hspace{0.3cm} foreach $u\in U$
\item \hspace{0.6cm}  if($F\wedge \neg v\wedge A|_{U -\{u\}}$ has unsatisfiable core $\boldsymbol{F^{US}}$) %\{
...
\end{theorem}
\begin{IEEEproof}
We need to prove by induction on the foreach statement in line 2 of algorithm 4.

For the base case,
according to line 5 of algorithm 3,
...
assume that when the algorithm 4 reachs the foreach statement in line 2,
$F\wedge\neg v\wedge A$ is unsatisfiable.
Then the if condition in line 3 of algorithm 4 may be:
\begin{enumerate}
...
By assigning $A|_{U-\{u\}}\cap F^{US}$ back to $A$ in line 4 of algorithm 4,
...
we need to find more efficient approaches to merge satisfying assignments.
...
the two satisfying assignments can't be merged by removing input variables.

But for a larger function such as $f^{-1}_v$ that contain XOR \textbf{MAY} gates $z=v_1 \oplus v_2$,
we can first check whether it actually exists,
and then merge these two satisfying assignments by replace $v_1$ and $v_2$ with $z$ in $A$.

To check discover???

Intuitively,
for an satisfying assignment $A$ that can force $v$ to be 1,
assume its domain is $U\subseteq O_{var}$,
for certain $v_1,v_2\in U$,
we can invert the value of $v_1$ and $v_2$ in $A$:

\begin{equation}
A_{\overline{v}_1,\overline{v}_2}=A_F|_{V-\{v_1,v_2\}}|^{v_1\to \neg A_F(v_1)}|^{v_2\to \neg A_F(v_2)}
\end{equation}

we can test whether $A_{\overline{v}_1,\overline{v}_2}$ can also force $v$ to be 1,
...
Thus,
$A_F$ and $A_{\overline{v}_1,\overline{v}_2}$,
...
Now,
the support set of $f^{-1}_v$ and $f^{-1}$ will change from $O_{var}$ to $O_{var}\cup \{x_1\}$.
...
\item \hspace{0.9cm}     $G_{new}= G_{new}\cup \{ x_1= v_1\oplus v_2\}$
\item \hspace{0.9cm}     $A_F= A_F|_{O_{var}-\{v_1,v_2\}}|^{x_1\to A_F(v_1)\oplus A_F(v_2)}$
\item \hspace{0.9cm}     $O_{var}= O_{var}\cup \{x_1\}-\{v_1,v_2\}$
\item \hspace{0.9cm}     $F_E= F_E\wedge bcls_{A_F}$
\item \hspace{0.9cm}     $F_E= F_E\wedge \bigwedge _{\{x_1= v_1\oplus v_2\}\in G_{new}}\big\{x_1\equiv v_1\oplus v_2\big\}$
...
and connect the output of $o_i$ to input of $o_{i-1}$.
...
assume the output variables set of all discovered XOR gates are $U_{xor}$,
then the support set of boolean function $f^{-1}$ will change from $O_{var}$ to $O_{var}\cup U_{xor}$.
As shown in left-top side of the figure \ref{reg_bank}.

So we need to instance all XOR gates discovered by the $XORMIN$ function.
...
\end{tabular}
\end{table}
Table \Rmnum{2} compares the following three statistics between the two algorithms proposed by Shen et al.\cite{ShegnYuShen:iccad09} and by us.
\begin{enumerate}
\item The three \textbf{time} rows compare the run time overhead of building complementary circuit.
Obviously,
our approach can be significantly faster than BFL only approach.
...
Table \Rmnum{3} compares the circuit area of hand written decoders and decoders built by Shen et al.\cite{ShegnYuShen:iccad09} and our algorithm.
...
\item There are no significant area difference between our algorithm and Shen et al.\cite{ShegnYuShen:iccad09}.
\end{enumerate}
...
this problem has been considered again \cite{DSGSYNTMPLG,SYNTMPLGSPC}.
But in 1989, A. Pnueli and R. Rosner\cite{SYNRCTVMD} pointed out that the complexity of LTL synthesis is double exponent in the size of the formula.
...
It's kernel is two-level logic minimization,
which tries to find a smaller sum-of-products expression for boolean function $f$.
...
One research direction focus on extending classical two-level AND-OR minimization to two-level AND-XOR network \cite{Mod2sum,ANDEXOR}.
...
%Download from  \url{},
IEEE Standard for Information technology¡ª
Telecommunications and information exchange between systems¡ª
Local and metropolitan area networks¡ª
Specific requirements¡ª
