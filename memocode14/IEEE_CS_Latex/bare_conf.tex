...
While the Cloud vendors can be trusted,
the virtual machines cannot.
...
Roy\cite{csRoy} and Fu \cite{csFu} suggest the possibility of extracting circuit information from CNF formula.
...
while our algorithms are presented in Section 4.
Section 5 analyzes correctness, effectiveness and algorithms complexity;
...
The Boolean value set is $B=\{T,F\}$.
...
Circuit structure extraction algorithms\cite{csRoy,csFu},
...
According to \cite{HV-grid}, since solutions to difficult instances of NP-complete problems are rare events,
...
When we design the Cloud oriented SAT solving framework, the following four goals are taken into consideration:
\begin{enumerate}
 \item 
%First,
...
Singular Husk formula is a CNF with only one solution,
and assignments of variables is non-uniform,
that is,
not all 0 or all 1.
\end{definition}
...
and assignments of variables in every solution are non-uniform.
\end{definition}
...
These algorithms will be described in Subsection \ref{genhusk}, \ref{obfuscating}
...
In this paper, Husks formula is constructed based on prime factorization method\cite{genSAT},
as shown in Algorithm \ref{algo2_gen}.
...
Variable of a gate's output is \textbf{output variable}.
\end{definition}
...
Fu\cite{csFu} presents another algorithm that
...
\\$\textbf{Fact 2:}$ Solution space should not be under-approximated after obfuscation, otherwise the result will be incorrect.
...
By CSA strategy, 
the scheme changes the clause set and literal set of $F_C$, 
...
We will describe them in Subsection \textit{\ref{embeded rules}}) and \textit{\ref{embeded strategy}}) respectively. 
...
% But partition based technique \cite{Partition} can easily separate the two undependent formulas. 
...
If this variable is $T$ in $R_H$, insert its negative literal into $c$;
Otherwise, insert its positive literal.
...
If this variable is $T$ in $R_H$, insert its positive literal into clause;
Otherwise, insert its negative literal.
...
For formula $F_C$, and satisfiable formula $F_H$ with $R_H$ as one of its assignments, 
...
For CNF formula $F_C$, Singular Husk formula $F_{_SH}$, if
...
According to Lemma \ref{HE} in Subsection \ref{correctness}, 
...
Thus, each solution of $F_O$ can be partitioned into solutions of $F_C$ and $F_H$. So
...
For each $S_{H_l}$, let $F_{slH}=(\bigwedge_{1\leqslant i\leqslant n}^{B_{l_i}\equiv T}y_{i})\wedge 
(\bigwedge_{1\leqslant j\leqslant n}^{B_{l_j}\equiv F}\neg y_{j})$,
and $F_{_SH}=\bigvee_{1\leqslant l\leqslant m}F_{slH}$,
we have $F_{_SH} \equiv F_H $.
\end{lemma}
...
\end{equation}
$S_{H_l}$ is one solution of $F_H$, so $F_H(S_H/V_{F_H}) $ is true.
So:
...
% \begin{proof}
Its proof is similar to Lemma \ref{HE}.
% \end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{lemma}[Singular Husk Equation(SHE)]\label{SHE}
% 
...
then $Obf(F_C,F_{_RH}\vee F_{_SH},R_H)\equiv Obf(F_C,F_{_RH},R_H) \vee Obf(F_C,F_{_SH},R_H)$
\end{lemma}
\begin{proof}
Assume
\begin{enumerate}
...
According to \textbf{Procedure} \ref{obsprocedure}, construct $F_O$ in 3 \textbf{steps}.
\begin{enumerate}
...
$Obf(F_C,F_{_RH} \wedge F_{_SH},R_H) \equiv Obf(F_C,F_{_RH},R_H) \wedge Obf(F_C,F_{_SH} ,R_H)$
\end{lemma}

% \begin{proof} 
Its proof is similar to Lemma \ref{ORrelation-Holding-Obfuscation}.
  % \textbf{Assume}
% \begin{enumerate}
% \item[-] Clause $A=a\vee X$,and clause $B=b$, while $b\notin X$,arbitary formula $F_{Cn},F_{_SH}$
...
According to \textbf{Procedure} \ref{obsprocedure}, 
construct $F_O$ in 3 \textbf{steps}.
\begin{enumerate}
...
For any CNF formula $F_C$, we have
 \textbf{$Obf(F_C,B\equiv\neg b,{b\equiv F})\equiv F_C\wedge \neg b$}
% % Lemma \ref{UNSSE-lemma} can be expressed as following:
\end{lemma}
% \begin{proof}

Its proof is similar to Lemma \ref{UPSSE-lemma}.
% \end{proof}
% \begin{proof}
% 
% \begin{enumerate}
...
Let's discuss SSE Obfuscation based on singular Husk formula,
and SSO Obfuscation based on Husks formula.

\textbf{Theorem \ref{SSEtheorem} Solution Space Equated (SSE) Obfuscation}
For CNF formula $F_C$, and Singular Husk formula $F_{_SH}$, if
\begin{enumerate}
 \item[-] $V_{F_C}\cap V_{F_H}\equiv \phi$, $R_H$ is unique solutions of $F_H$.
...
According to \textbf{Procedure} \ref{obsprocedure}, constuct $F_O$ in 2 \textbf{steps}:
\begin{enumerate}
\item \label{Fop}  
Let $F_{Op}=F_C$.  
...
For CNF formula $F_C$, Husks formula $F_H$, if
\begin{enumerate}
...
Assume 
        a solution of $F_H$ is $R_H=\{y_i=B_{R_k}|B_{R_k} \in \{T,F\}, 1\leqslant k\leqslant n\}$,
        and all other $m-1$ solutions $\{S_{H_l} | 1\leqslant l\leqslant m-1\}$.
        $S_{H_l}=\{y_k=B_{l_k}|B_{l_k}\in \{ T,F \},~1\leqslant k\leqslant n\}$.   
...
With Equation (\ref{SSOE5}), we have:
	...
	\end{equation}
Since $F_{_RH}$ is satisfiable, $V_{F_C}$ $\cap$ $V_{F_H}$ = $\phi$:
...
According to Theorem \ref{SSOtheorem},
the solution space after SSO obfuscation is an overapproximation of the original one.
So even SAT solver can't find out the real solution.
...
\section{Experiments}
Algorithms are implemented in language $C$.
The experiments are conducted on a laptop with Intel Core i7 CPU @ 2.00GHz and 8GB RAM.
We use MiniSat as solver.

We unroll circuits in ISCAS89 benchmark for 100 times,
...
$~~$\textbf{Marked Gate}:number of gates being changed.\\
...
As for Asymmetric Speedup\cite{c.WANG} defined in Equation (\ref{asyncspeedup}), 
for more than 60\% of circuits, the value is more than 260\%.
...
\begin{equation}\label{asyncspeedup}
...
These facts remind us of two issues: 
...
much more attention should be pay on the impact on solving time. 
%\begin{figure}
%\centering
