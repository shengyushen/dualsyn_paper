...
that can be exploited to overcome this problems.
% }
...
For example,
$\vec{stg}^j$ is the vector of registers in the $j$-th pipeline stage.
...
output sequence $<o_{p},\dots,o_{p+l+r}>$ uniquely determine all $i\in \vec{i}_{p+l}$.
...
cat be uniquely determined by $\vec{stg}^{j+1}$ at $j-((n-2)-(p+r-1))+1$-th step.
...
Similar to the last subsection,
...
The 2nd to 3rd column of shows respectively the number of inputs, outputs and registers of each benchmark.
...
In this paper, 
all areas, gate number and delay are obtained in the same setting.
...
Table \ref{tab_res} compares the old algorithm from \cite{ShenTCAD10} to this paper's algorithm.
The 2-th to 4-th columns show respectively the run time of \cite{ShenTCAD10}'s algorithm to generate the decoder without pipeline,
...
By comparing the 3rd and the 6-th column,
we can find that the decoders' delay have been improved significantly.
And the the last column shows that there actuall exist very deep pipeline,
especially for the t2eth benchmark,
which have 4 pipeline stages.
...
We study their code and confirm that they actually dont have such pipeline stages.
Their area are so large because they use a much more compilcated encoding schema than other encoders.
...
\hline
 ...
 Names     &   \multicolumn{3}{|c|}{ated by \cite{ShenTCAD10}}         &   \multicolumn{4}{|c|}{ated by this paper} \\\cline{2-8}
 ...
 t2eth     & 12.71          & 6.54                      & 434          & 47.19               &  6.12                     & 877   &  8/8/    \\
           &                &                           &              &                     &                           &       &  10/20    \\\hline
 scr.      &                \multicolumn{7}{|c|}{no pipeline structure found}                                                           \\\hline
 xfi       &                \multicolumn{7}{|c|}{no pipeline structure found}\\\hline
