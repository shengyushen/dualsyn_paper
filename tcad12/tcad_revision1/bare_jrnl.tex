...
% To avoid this tedious \textbf{task},
...
To avoid this tedious \textbf{task},
we propose an automatic approach to infer this assertion,
by iteratively discovering and removing cases without decoders.

% However,
% under this inferred assertion,
% multiple decoders may exist simultaneously.
% \textbf{Therefore},
% we propose a second algorithm to discover them,
% by iteratively testing whether $\mathbb{R}$,
% the Boolean relation that \textbf{uniquely determines} the encoder's input,
...
To discover all decoders that may exist simultaneously under this assertion,
a second algorithm based on functional dependency is proposed to decompose $\mathbb{R}$,
the Boolean relation that uniquely determines the encoder's input,
into all possible decoders.

To help the user \textbf{select} the correct decoder,
a third algorithm is proposed to infer each decoder's precondition formula,
which represents those cases that lead to this decoder's existence.
...
Moreover,
when multiple decoders exist simultaneously,
the user can easily select the correct one by inspecting their precondition formulas.
\end{abstract}
...
One of the most difficult \textbf{tasks} in designing communication chips
...
\textbf{To manually specify an assertion,
...
which had already been experienced by me in \cite{ShengYuShen:iccad09}.}
To avoid this tedious \textbf{task},
in this study,
...
we propose a second algorithm to discover all these decoders by iteratively testing whether $\mathbb{R}$,
the Boolean relation that uniquely determines the encoder's input,
functionally depends on those discovered decoders.
For every configuration value that fails this test,
...
one only needs to find out the meaning of these \textbf{TWO} pin,
...
For every configuration letter $A(c)$ found in Line \ref{algo_pcln_Ac} that can lead to the decoder's nonexistence,
...
\textbf{$InferCoveringFormula$ uses cofactoring \cite{Cofact} to assert the satisfying assignments of $i_{n-p}^{n+d-1}$,
...
An earlier study \cite{ShengYuShen:iccad11} had proved that this algorithm is a halting one.
...
\textbf{$\mathbb{R}(c,X,Y)$ defines a mapping from $c$ and $X$ to $Y$,
...
Thus,
our \textbf{task} here is to find out the set $\{\mathbb{R}_1,\dots,\mathbb{R}_n\}$ step--by--step.
...
\begin{split}
Y&=f(c,X)=\bigvee _{i=1}^{m} \{IA_i(c)\wedge Y_i\}=\bigvee _{i=1}^{m} \{IA_i(c)\wedge Y'_i\}=Y'
\end{split}
\end{equation}
...
Line number of&&&&&\\
verilog source&214&466&24&1139&1073\\\hline
...
\textbf{The fourth row shows the total runtime of our new algorithm},
while the fifth row shows the value of the discovered $d$, $p$, and $l$.
The last row shows the number of discovered decoders.
...
this&Runtime                             &3.83    &3841.34   &18.73     &8.51    &1791.22      \\\cline{2-7}
paper&$d,p,l$                            &1,5,1   &0,5,2     &0,4,2     &2,5,1   &4,5,1          \\ \cline{2-7}
&\#decoders                              &1       &2         &2         &1       &1          \\ \hline
\end{tabular}\label{tab_res}
\end{table}
The third and the fifth rows indicate that there are some minor differences in those parameter values,
which is caused by the difference between the embedded assertions and inferred assertion.
The latter contains much more configuration letters than the former.
...
\texttt{((link\_up\_loc\&!reset\_tx\&
!txd\_sel[1]\&!jitter\_study\_pci[1]\&!txd\_sel
[0]\&!jitter\_study\_pci[0])|(!link\_up\_loc\&
!reset\_tx\&!txd\_sel[1]\&tx\_enc\_conf\_sel[3]\&
tx\_enc\_conf\_sel[2]\&!jitter\_study\_pci[1]\&
!txd\_sel[0]\&!jitter\_study\_pci[0])|(!link\_up
\_loc\&!reset\_tx\&!txd\_sel[1]\&tx\_enc\_conf\_
sel[3]\&!tx\_enc\_conf\_sel[2]\&tx\_enc\_conf\_
sel[1]\&!jitter\_study\_pci[1]\&!txd\_sel[0]\&
!jitter\_study\_pci[0])|(!link\_up\_loc\&
!reset\_tx\&!txd\_sel[1]\&!tx\_enc\_conf\_sel[3]
\&!tx\_enc\_conf\_sel[2]\&tx\_enc\_conf\_sel[1]\&
!jitter\_study\_pci[1]\&!txd\_sel[0]\& !jitter
\_study\_pci[0]\&tx\_enc\_conf\_sel[0]))}
...
the user need to inspect $\{IA_1,\dots,IA_{m}\}$ to select the correct one.
...
The runtime problem was addressed by simplifying the SAT instance with unsatisfiable core extraction\cite{ShengYuShen:tcad} and Craig interpolation \cite{Roland:iccad11},
while the halting problem was handled by detecting loops \cite{ShengYuShen:tcad11,Roland:iccad11}.
