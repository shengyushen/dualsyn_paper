...
be uniquely determined by its output variables.
However,
many modern encoders employ flow control mechanism that 
can not be handled by all these algorithms.
When the encoder's output sequence is too fast to be processed by the decoder,
to prevent losing data,
the encoder with flow control mechanism will stop outputting valid data symbol,
but instead output an idle symbol that can only uniquely determine a subset of the encoder's input variables.
And the decoder should recognize this idle symbol and discard it.
Although this mechanism can prevent the faster encoder from overwhelming the slower decoder,
the fact that some input variables can not be uniquely determined by the idle symbol 
fail the assumption of all complementary synthesis algorithm.

This paper propose the first algorithm 
...
\textbf{By assuming that $\vec{i}$ can always be
...
can automatically generate the encoders' decoder.}

\textbf{
As shown in Figure~\ref{fig:nonuniq}a),
todays high speed communication systems
% such as Ethernet \cite{WIKIether} and PCI Express \cite{WIKIpcie},
employ flow control mechanism \cite{WIKIfc} in the encoder and decoder to 
prevent the faster transmitter from overwhelming the slower receiver.
% transmit several Gigabits per second.
% It is physically impossible to 
% distribute so fast a clock at Gigahertz to both the encoder and decoder.
% So as shown in Figure~\ref{fig:nonuniq}a),
% the encoder and decoder must be driven by two clocks with slightly different frequencies.
% To prevent the faster encoder from overwhelming the slower decoder,
% flow control mechanism \cite{WIKIfc} is employed in the encoder to 
% insert some control symbols $C$s into the sequence of data symbols $D_j$s.
% Such control symbols C are defined by the standard of communication systems,
% such as the K28.5 symbol defined in clause 36 of IEEE 802.3 standard\cite{IEEE8023_S3}.
In such systems,
the encoder inserts some control symbols $C$s 
into the sequence of data symbols $D_j$s.
The communication system standards guarantee 
that $C$ can never be equal to any $D_j$.
So the decoder can recognize and discard all $C$s.
In this way,
the speed difference between transmitter and receiver can be compensated for.
}
...
\textbf{
However,
such encoder can not be handled by state-of-the-art complementary synthesis algorithms,
because $C$ can not be used to uniquely determine the value of $i$.
Figure~\ref{fig:nonuniq}b) shows the pseudo code of such 
an encoder with flow control mechanism.
Its input variables $\vec{i}$ are partitioned into two sets:
the input data variables $\vec{d}$ to be encoded,
and the flow control variables $\vec{f}$ indicating 
the validness of $\vec{d}$ with a predicate $valid(\vec{f})$.}

\begin{enumerate}
\item When $valid(\vec{f}) \equiv 1$, 
the encoder maps $\vec{d}$ with an encoding function $enc$ 
to the output variables $\vec{o}:=enc(\vec{d})$.
In this case $\vec{d}$ and $\vec{f}$ can both be uniquely determined 
by a bounded sequence of $\vec{o}$.

\item \textbf{However,
when $valid(\vec{f}) \equiv 0$,
the encoder send some control symbol $C$ to $\vec{o}$ 
without considering the value of $\vec{d}$.
So in this case,
$\vec{d}$ can not be uniquely determined.
This prevent all current complementary synthesis algorithms from handling this encoder.
On the other hand,
sending $C$ to $\vec{o}$ without considering $\vec{d}$
means that $\vec{d}$ is meaningless in this case,
and the encoder actually do not need the decoder to recover $\vec{d}$.
Thus,
to handle such an encoder,
we should exclude the case $valid(\vec{f}) \equiv 0$.}
\end{enumerate}
...
% Thus,
% none of the current complementary synthesis algorithms can handle such an encoder.
