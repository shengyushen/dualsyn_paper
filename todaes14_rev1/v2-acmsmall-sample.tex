...
\textbf{
However, 
the encoders of many high speed communication systems employ flow control mechanism that 
fail this assumption.
...
the transmitter will drop $f$ to 0 to stop transmitting new $d$,
which makes the encoder output the idle symbol $I$ to $\vec{o}$ without considering the value of $d$.
And the decoder should discard this idle symbol $I$,
and send $f\equiv 0$ to the receiver with whatever value on $d$.}

\textbf{
Although this mechanism can prevent data from losing,
it fail the assumption of all current complementary synthesis algorithms,
...
\textbf{Line 5 and 6 are the assertion predicates given by the user that constrain the valid valuation on $\vec{i}$.}
...
So,
Algorithm \ref{algo_remove} is used to minimize $<p,d,l>$ before passing it to other algorithms.
}
...
So we first present an intuitive introduction in \ref{subsub_intro}.
...
\subsubsection{}\label{subsub_intro}\textbf{Intuitive introduction}

In this Section,
what we want is the predicate $valid(\vec{f})$ that enables $\vec{d}$ to be uniquely determined.

For every particular valuation of $p$,$l$ and $r$,
we can use Algorithm \ref{alg_craigchar} to characterize the set of 
satisfying assignments on $\vec{f}$ that makes $F_{PC}(p,l,r)$ satisfiable.
We call the predicate that cover and only cover this set of satisfying assignments $FSAT_{PC}(p,l,r)$.
...
% On the other hand,
$\vec{d}$ not uniquely determined for some particular $p$,$l$ and $r$,
may become uniquely determined for larger $p'$,$l'$ and $r'$.
That means,
a particular valuation of $\vec{f}$ that 
makes $F_{PC}(p,l,r)$ satisfiable for some $p$,$l$ and $r$,
may also make it unsatisfiable for some larger $p'$,$l'$ and $r'$.

So $FSAT_{PC}(p,l,r)$ is only an over-approximation of $valid(\vec{f})$.
We still need an under-approximation to construct a halting algorithm.
...
Although the general framework is correct,
there are two problems for this algorithm:
\begin{enumerate}
 \item It may incorrectly rule out some valuation of $\vec{f}$ that can actually 
 enable $\vec{d}$ to be uniquely determined.
 That is,
 $\vec{d}$ that can not be uniquely determined with smaller $p$,$l$ and $r$,
 may become uniquely determined with larger $p$,$l$ and $r$.
 For example, assume we are handling an encoder with latency of 3 steps.
 Then for any valuation of $p$,$l$ and $r$ that is smaller than 3, 
 $\vec{d}$ can never be uniquely determined.
 But for $p$,$l$ and $r$ that are larger than 3,
 $\vec{d}$ can become uniquely determined.
 \item When to terminate this loop.
\end{enumerate}

Actually these two problems can be answer with the same mechanism.



\subsubsection{}\label{subsub_nonloop}\textbf{Inferring the non-looping predicate}
\\
\\
...
\subsubsection{}\label{subsub_loop}\textbf{Inferring the looping predicate}
...
\subsubsection{}\label{subsub_overal}\textbf{The overall algorithm framework to characterize the predicate $valid(\vec{f})$.}
...
And then Algorithm \ref{algo_pcln} costs 2.067509 seconds to infer the predicate $CNTL\_TXEnable\_P0\equiv 1$ that enables the data vector to be uniquely determined.
Finally, 
...
And then Algorithm \ref{algo_pcln} costs 1.443065 seconds to infer the predicate $bad\_code\equiv 0$ that enables the data vector to be uniquely determined.
...
And then Algorithm \ref{algo_pcln} costs 27.456717 seconds to infer the predicate $tx\_enc\_ctrl\_sel\equiv `PCS\_ENC\_DATA$ that enables the data vector to be uniquely determined.
