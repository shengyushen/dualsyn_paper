...
The encoder maps its input variables $\vec{i}$ to its output variables 
$\vec{o}$ according to some predefined rules,
such as Ethernet \cite{WIKIether} and PCI Express \cite{WIKIpcie},
while the decoder recovers $\vec{i}$ from $\vec{o}$.
Complementary synthesis \cite{ShenICCAD09} facilitates this job by 
automatically generating an encoders' decoder.

\textbf{However,
State-of-the-art complementary synthesis algorithms
[\citeNP{ShenICCAD09};\citeyearNP{ShenTCAD10,ShenTCAD11,ShenTCAD12};\citeNP{LiuICCAD11};\citeyearNP{LiuTCAD12};\citeNP{TuDAC13}] 
all assume that $\vec{i}$ can always be
uniquely determined by a bounded sequence of $\vec{o}$.
This assumption is too restrictive for todays high speed communication systems,
such as Ethernet \cite{WIKIether} and PCI Express \cite{WIKIpcie},
that transmit several gigabits per second.}
It is physically impossible to distribute so fast a clock to both the transmitter and receiver.
\textbf{So as shown in Figure~\ref{fig:nonuniq}a),
the transmitter and receiver} must be driven by two clocks with slightly different frequencies.
...
insert \textbf{some predefined control symbols} into the sequence of input variables $\vec{i}$,
