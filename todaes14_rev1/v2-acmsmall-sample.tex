...
\subsection{Incremental SAT mechanism of MiniSAT solver}
...
The major changes are the two newly added $addClause$ in Line \ref{addcls21} and \ref{addcls22},
and the two newly added $solve$ in Line \ref{solve21} and \ref{solve22}.
...
\For{$r'=r \to 1$} {
  \If{$F_{PC}(p,l',r'-1)$ is satisfiable} {
    break
  }
  \ElseIf{$r'\equiv 1$} {
    $r':=r'-1$\\
...
This algorithm handle $p$,$d$ and $l$ similarly, 
so I only explain $l$ here.
For each $l'$,
$l'-1$ is used to checked for $F_{PC}$'s satisfiability.
