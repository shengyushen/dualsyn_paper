...
Here,
$p$ is the length of the prefix state transition sequence that leads the encoder into a set of state
that enable $i$ to be uniquely determined.
...
\textbf{One important fact here is that the unrolled transition relation does not include an initial state.
This leads to two major advantages:
First,
it simplify and speedup our algorithm by avoid the need to compute the reachable state set or inductive in.
...
That is,
if requires that $\vec{i}$ must be uniquely determined on all states that is reachable in $p$ steps from any states,
instead of on all reachable states.
Tu and Jiang \citeyear{TuDAC13} propose a breakthrough algorithm that handle this problem by inferring inductive invariants.
That work is orthogonal to ours,
...
the way it handle the invalid data vector is different.
