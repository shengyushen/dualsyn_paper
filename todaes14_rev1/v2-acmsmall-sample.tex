...
Complementary synthesis \cite{ShenICCAD09}
...
But it fail the assumption of all current complementary synthesis algorithms,
...
As shown in Figure \ref{fig_pcln}a),
...
Line 1 of Equation (\ref{uniqt1}) corresponds to the path in Figure \ref{fig_pcln}a),
...
\textbf{According to Figure \ref{fig_pcln},
it is obvious that,
...
if $F_{PC}(p,l,r)$ is unsatisfiable,
then $i_{p+l}$ can be uniquely determined.
...
for this particular combination of $p$, $l$ and $r$.
...
One such solution is shown in Figure \ref{fig_pcln}b),
which is similar to Figure \ref{fig_pcln}a) but with three additional constraints to detect loops 
...
Inspired by the Figure \ref{fig_pcln}b) and $F_{LN}(p,l,r)$,
...
by unrolling the three loops in Figure \ref{fig_pcln}b),
