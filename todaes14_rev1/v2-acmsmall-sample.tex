...
We will then introduce how to remove the redundancy of $p$, $l$ and $r$ in Subsection \ref{reduceing},
and finally present another possible structure of our algorithm in Subsection \ref{alter},
and discuss why we have chose the one in Subsection \ref{basic} instead of the one in Subsection \ref{alter}.
...
\textbf{According to Figure \ref{fig_pc},
it is obvious that $p$ does not affect the decoder's logic complexity and circuit area.
So we do not minimize its valuation here.
}
...
and then minimize their valuations with Algorithm \ref{algo_remove}.
...
The \#gate and area columns show the number of gates and area of the encoder when mapped to mcnc.genlib library with
...
\hline
                 & Names     &\#in/\#out &\#reg /\#gate& area & description                          & How to handle\\\hline\hline
Benchm-          & pcie      & 10 / 11   & 22   / 149  & 326  &PCIE 2.0 encoder                      & Subsection \ref{subsec_pcie2}\\\cline{2-7}
arks in (1)      & xgxs      & 10 / 10   & 17   / 249  & 572  &10G Ethernet encoder in clause        & Subsection \ref{subsec_10g}\\
with flow        &           &           &             &      & 48 of IEEE-802.3ae\cite{IEEE8023_S4} &  \\\cline{2-7}
control          & t2ether   & 14 / 14   & 53   / 427  & 947  &Ethernet encoder in                   & Subsection \ref{subsec_t2e}\\
                 &           &           &             &      &UltraSPARC T2 processor               &\\\hline\hline
Benchm-          & xfi       & 72 / 66   & 72   / 5086 & 12381&10G Ethernet encoder in clause        &\\
arks in (1)      &           &           &             &      & 49 of IEEE-802.3ae\cite{IEEE8023_S4} &\\
without flow     & scrambler & 64 / 64   & 58   / 353  & 1034 & Transforming a bit stream to         &\\
control          &           &           &             &      & have lots of 01 flipping             &Comparing in \\\cline{1-6}
Benchm-          & CC\_3     & 1  / 2    & 3    / 5    & 13   &convolution code                      &Subsection \ref{subsec_nonflow}\\
arks in (2)      &           &           &             &      & with length 3                        &with \cite{LiuTCAD12}\\\cline{2-6}  
and (3)          & CC\_4     & 1  / 2    & 4    / 7    & 19   &convolution code                      &\\
                 &           &           &             &      &with length 4                         &\\\hline
                 & CC\_13    & 1  / 2    & 13   / 36   & 105  &convolution code                      &\\
                 &           &           &             &      &with length 13                        &\\\cline{2-6}
Benchmarks       & CC\_14    & 1 / 2     & 14   / 28   & 84   &convolution code                      &\\
in (3) but       &           &           &             &      &with length 14                        &\\\cline{2-6}
not in (2)       & & 1  / 1    & 1    / 1    & 1    &The electrical idle state\\\cline{2-6}
                 & & 1  / 1    & 1    / 1    & 1    &The electrical idle state\\\hline
\end{tabular}}
\end{table}%



The three benchmarks pcie, xgxs and t2ether shown in Row 2-4 of Table \ref{tab:summ} have flow control mechanisms.
They will be respectively discussed in Subsection \ref{subsec_pcie2},\ref{subsec_10g} and \ref{subsec_t2e}.

The following two benchmarks xfi and scrambler in Row 5-6 of Table \ref{tab:summ} have no flow control mechanisms.
We will compare them 


By studying the 5 benchmarks used in our previous papers [\citeNP{ShenICCAD09};\citeyearNP{ShenTCAD10,ShenTCAD11,ShenTCAD12}],
...
because these benchmarks all come from real industrial projects.
 ...
 There are 5 benchmarks in this set.
 Three of them,  have flow control mechanism.
 The other two, xfi and scrambler, do not have.
 So we will present 
 about how to handle their flow control mechanism.
 This is also the major contribution of this paper.
 
For all other benchmarks in Table{tab:summ} without flow control mechanism,
we will compare them in Subsection \ref{subsec_nonflow},
between using our algorithm and \cite{LiuTCAD12}.
...
We will present the experimental result for them in Subsections \ref{subsec_pcie2},\ref{subsec_10g} and \ref{subsec_t2e}.

On the other hand,
for those benchmarks without flow control mechanism,
their input variables can always uniquely determined.
So our algorithm can recognize all their input variables as flow control variables,
...
But our algorithm can also handle them,
because they will make Algorithm \ref{alg_fofc} to find that there 
...
Algorithm \ref{alg_fofc} identifies the flow control variable $CNTL\_TXEnable\_P0$.
And then Algorithm \ref{algo_infer} infers the predicate $CNTL\_TXEnable\_P0\equiv 1$ that enables the data vector to be uniquely determined.
% Finally, 
% with the inferred predicate,
% generating the decoder's Boolean functions for $CNTL\_TXEnable\_P0$, $TXDATA$ and $TXDATAK$ costs 3.8892 seconds.
% The area of decoders with and without Algorithm \ref{algo_remove2} are presented in Table \ref{}.
% After being mapped to LSI10K library,
...
Algorithm \ref{alg_fofc} identifies the flow control variable $bad\_code$.
And then Algorithm \ref{algo_infer} infers the predicate $bad\_code\equiv 0$ that enables the data vector to be uniquely determined.
...
Algorithm \ref{alg_fofc} identifies the flow control variables $tx\_enc\_ctrl\_sel$, $tx\_en$ and $tx\_er$.
And then Algorithm \ref{algo_infer} infers the predicate $tx\_enc\_ctrl\_sel\equiv `PCS\_ENC\_DATA$ that enables the data vector to be uniquely determined.
...
and then reduce them with Algorithm \ref{algo_remove}.
...
\hline
\multicolumn{2}{|c|}{}          & \multicolumn{5}{|c|}{A1:Increasing simultaneously   }      &  \multicolumn{5}{|c|}{A2:Increasing separately} \\\cline{3-12}
\multicolumn{2}{|c|}{benchmarks}& p,l,r& time       & time            & time       &  total  & p,l,r& time       & time            & time       & total  \\
\multicolumn{2}{|c|}{}          &      & find.      & infer.          & minimiz.   &  run    &      & find.      & infer.          & minimiz.   & run \\
\multicolumn{2}{|c|}{}          &      & $\vec{f}$  & $valid(\vec{f})$& $p$,$l$,$r$&  time   &      & $\vec{f}$  & $valid(\vec{f})$& $p$,$l$,$r$& time \\\hline\hline
Flow    &PCIE2                  & 3,0,2& 0.49       & 1.21            & 0.68       &  2.38   & 3,0,2& 0.38       & 0.80            & 0.38       & 1.60 \\\cline{2-12}
Con-    &Eth10G                 & 3,0,1& 0.31       & 0.88            & 0.52       &  1.71   & 3,0,1& 0.23       & 0.58            & 0.30       & 1.11 \\\cline{2-12}
trol    &T2Eth                  & 4,0,4& 4.28       & 15.17           & 6.25       &  25.70  & 4,0,4& 15.47      & 13.85           & 6.19       & 35.51 \\\hline\hline
Non-    &XFI                    & 2,1,0& 4.59       & 3.60            & 9.55       & 17.74   & 2,1,0& 3.52       & 2.75            & 10.05      & 16.32 \\\cline{2-12}
Flow    &Scrambler              & 2,1,0& 0.64       & 0.58            & 1.33       & 2.55    & 2,1,0& 0.48       & 0.43            & 1.47       & 2.38 \\\cline{2-12}
Con-    &CC\_3                  &      &            & 7146            & 7146       &         & 5,0,4& 19.92      & 7146            & 7146       &  \\\cline{2-12}
trol    &CC\_4                  &      &            & 7146            & 7146       &         & 5,0,4& 19.92      & 7146            & 7146       &  \\\hline
\end{tabular}}
\end{table}%
...
\subsection{Comparing run times and circuit areas for the two cases with and without Algorithm \ref{algo_remove2}}
...
we present the experimental result in Table \ref{tab:sum}.
...
\begin{table}[b]%
\tbl{The summary of experimental results\label{tab:sum}}{%
\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|c|c|}
\hline
       & run       & run              & \multicolumn{4}{|c|}{Without Alg. \ref{algo_remove2}} &  \multicolumn{4}{|c|}{With Alg. \ref{algo_remove2}} \\\cline{4-11}
bench- & time      & time             & p,l,r& runtime    & decoder & \#                      & p,l,r& runtime    & decoder & \#               \\
marks  & find.     & find.            &      & generat.   & area    & of                      &      & generat.   & area    & of               \\
       & $\vec{f}$ & $valid(\vec{f})$ &      & decoder    &         & reg                     &      & decoder    &         & reg              \\\hline\hline
PCIE2  & 1.04      & 1.86             & 4,4,4& 2.77       & 1642    & 22                      & 4,0,2&4.05        & 2363    & 0                \\\hline
Eth10G & 0.45      & 1.39             & 4,4,4& 2.03       & 1538    & 30                      & 4,0,1&3.01        & 1484    & 0                \\\hline
T2Eth  & 12.37     & 21.58            & 5,5,5& 19.92      & 7146    & 22                      & 5,0,4&34.29       & 5212    & 9                \\\hline
\end{tabular}}
\end{table}%
...
\begin{table}[t]%
\tbl{The summary of experimental results\label{tab:comparing_hand}}{%
\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|c|c|}
\hline
