...
a novel halting algorithm is proposed in this paper to check the existence of the decoder.
For every unfolded instance of the encoder's transition function,
this algorithm not only check whether the encoder's input can be uniquely determined by its output,
but also check whether there exists a loop-like path that reaches a particular state,
from which two more successive loop-like paths can produce an output sequence that corresponds to two different input letters.
These two checks will confirm respectively the existence and non-existence of the decoder,
and thus ensure the halting character of the new algorithm.

To illustrate its usefulness and efficiency,
this algorithm had been run on several complex encoders,
including PCI-E and Ethernet.
Experimental results indicate that
the new algorithm always halts properly by distinguishing correct encoders from the incorrect ones,
and it can be more than three times faster than previous work.
\end{abstract}
...
by checking Parameterized Complementary Condition(PC),
that is,
...
Another algorithm had been proposed in \cite{ShengYuShen:fmcad10}
to solve this problem by constructing an onion-ring between $PC$ and its over-approximation,
and checking whether $E$ is in all these rings.
This new algorithm is very slow and complicated.
So yet another halting algorithm is proposed in this paper,
which is both faster and more straightforward:

For every unfolded instance of $E$'s transition function,
this algorithm not only check whether the encoder's input can be uniquely determined by its output,
but also check whether there exists a loop-like path that reaches a particular state,
from which two more successive loop-like paths can produce an output sequence that corresponds to two different input letters.
These two checks will confirm respectively the existence and non-existence of $E^{-1}$,
and thus ensure the halting character of the new algorithm.

This algorithm had been implemented with the OCaml language,
...
\textbf{There is one issue that needs to be clarified.
...
The state, the input letter and the output letter at the $n$-th cycle are respectively denoted as $s_n$, $i_n$ and $o_n$.
The sequence of state, input letter and output letter from the $n$-th to the $m$-th cycle are respectively denoted as $s_n^m$, $i_n^m$ and $o_n^m$.
...
In this paper,
a similar concept: the \textbf{uninitialized state variables recurrence diameter} with respect to $M$,
$uirrd(M)$,
is defined as the longest state sequence without loop in  $M$.
%}
...
\textbf{According to our previous research \cite{ShengYuShen:iccad09},
and our engineering experience in designing communication chips for the TH-1A supercomputers\cite{TH1A},
many communication protocols are loseless,
that is,
...
and $p$ is the length of the prefix state transition sequence used to rule out some unreachable states.
Thus,
as shown in Figure \ref{t1},
the parameterized complementary condition($PC$)\cite{ShengYuShen:iccad09} can be defined formally as:
...
$i_n$ can be uniquely determined by $o_{n+d-l}^{n+d-1}$ on the state transition sequence $s_{n-p}^{n+d-1}$.
...
This non-halting algorithm\cite{ShengYuShen:iccad09} just iterate through all valuations of $d$, $l$ and $p$,
from small to large,
until one valuation of $d$,$l$ and $p$ that makes Equation (\ref{uniqt1}) unsatisfiable is found.
...
At the same time,
in Table \ref{tab_prodes},
XFI and scrambler also have $l$ larger than $d$.}

\begin{figure}[t]
\begin{center}
\includegraphics[width=0.3\textwidth]{mealy_add}
\end{center}
\caption{The circuit that break the causal relation}
  \label{mealy_add}
\end{figure}

\textbf{This problem will be explained with the circuit in Figure \ref{mealy_add}.
Assume its transition function $T$ is defined as :}
\begin{equation}
\end{equation}

\textbf{Intuitively,
this circuit add its input to its current state,
and put the result to its output and next state.
So,
it is obvious that,
...
But how to determine the non-existence of $E^{-1}$ is not defined.
So the key to a halting algorithm is to find out a sufficient and necessary condition for the non-existence of $E^{-1}$.
...
another tuple $<p',d',l'>$ with $p'>p$,$l'>l$ and $d'>d$ can always been found,
...
except that three new constraints are inserted to detect loops on state sequences $s_{n-p}^{n+d-l}$,$s_{n+d-l}^n$ and $s_{n+1}^{n+d}$.

\textbf{If this SAT instance is satisfiable,
for any parameter value $<p,d,l>$,
these three loops can be unfolded until $<p',d',l'>$ that is larger than $<p,d,l>$ is found.
It is obvious that this unfolded instance is still satisfiable,
which means $E\vDash PC(p',d',l')$ does not hold.
...
so these loops must be detected on both of them,
i.e.,
...
Thus,
the loop-like non-complementary condition is defined below to determine the non-existence of $E^{-1}$:
...
we have $F_{LN}(p,d,l)\to F_{LN}(p",d",l")$
\end{lemma}
\begin{proof}
According to Equation (\ref{uniqln}) and Figure \ref{fig_double_loop_unfold},
this is obvious.
\end{proof}
...
Assume that $E\vDash LN$ and $E\vDash PC$ both do not hold.
...
This means that there are loops in all these three paths,
...
$bound$ is the total length of the unfolded transition function.
Placing the enumeration of $bound$ on the out most loop
can avoid generating the SAT instance of unfolded transition function repeatedly for $F_{PC}$ and  $F_{LN}$.
And then,
in line 2,3 and 4,
the valuation of $p$,$d$ and $l$ are enumerated respectively.
...
\ENDFOR
\FOR{$l'=l-(d-d') \to 0$}
  \IF{$F_{PC}(p',d',l'-1)$ is satisfiable}
    \STATE break
  \ENDIF
\ENDFOR
...
which will cause unnecessary large overheads on the circuit area and run time of characterizing.
...
larger $p$ and $d$ lead to larger SAT instances for the characterization algorithm.
...
Table \ref{tab_info} shows information of the following benchmarks.
\begin{enumerate}
...
\subsection{Determining the existence of the decoder for Properly Designed Encoders}
\begin{table}[t]
\centering
...
&                                        &         &          &mbler     &        &hernet\\ \hline
&time chk                           &&&&&\\
\cite{ShengYuShen:fmcad10}&$PC$(sec)     &1.06     &70.52     &5.74      &2.40    &66.37\\\cline{2-7}
&$d,p,l$                                 &1,1,1    &0,3,2     &0,2,2     &2,1,1   &4,1,1         \\ \hline\hline
This&time chk                         &&&&&\\
paper&$PC$(sec)                           &0.39     &21.31     &3.27      &0.61    &65.34\\\cline{2-7}
&$d,p,l$                                 &1,2,1    &0,3,2     &0,2,2     &2,2,1   &4,2,1          \\ \hline
...
Table \ref{tab_cmparea} compares the circuit area of hand-written decoders,
and decoders built by this paper's algorithm.
...
\hline
                   &XGXS      &XFI       &scrambler    &PCI-E  &T2 et-\\
&&&&&hernet\\ \hline
hand-written       &921       &6002      &1629         &852   &1446          \\
decoders           &&&&&\\ \hline
decoders built by th-     &700       &12754     &1455         &455   &552          \\
is paper's algorithm   &&&&&\\ \hline
...
for the XFI case,
circuit area of this paper's algorithm is about 2 times larger than that of hand-written decoders.
...
decoders           &&&&&\\ \hline
decoders built by th-     &11.96       &28.13     &6.54         &9.09   &12.69          \\
is paper's algorithm   &&&&&\\ \hline
...
\hline
                                        &XGXS     &XFI       &scra-     &PCI-E    &T2 et-\\
                                        &         &          &mbler     &        &hernet\\ \hline
\cite{ShengYuShen:fmcad10}(sec)     &0.98     &35.08     &2.54      &1.36    &17.39\\\hline
This paper's                         &&&&&\\
algorithm(sec)                           &0.33     &14.44     &2.43      &0.56    &10.14\\\hline
\end{tabular}\label{tab_impdes}
\end{table}


\textbf{Table \ref{tab_cmptiming} compares the critical path latencies of hand-written decoders,
and decoders built by this paper's algorithm.
Their synthesis settings are same as Subsection \ref{subsec_area}.
According to Table \ref{tab_cmptiming},
for all those circuits,
the critical path latencies of the decoders built by this paper's algorithm are better than
that of those hand-written decoders.}
...
\subsection{Determining the non-existence of the decoder for Improperly Designed Encoders}
...
\textbf{The halting problem are handled by building a set of over-approximations that are similar to onion-rings \cite{ShengYuShen:fmcad10}.
The second shortcoming are addressed by simplifying the SAT instance with unsatisfiable core extraction\cite{ShengYuShen:tcad}.}
...
\textbf{Initial work on deriving program inverse used proof-based approaches\cite{prog_inv},
but it can only handle very small programs and very simple syntax structures.}

\textbf{Gl\"{u}ck et.al \cite{mtd_autoProginv} inverses the first-order functional programs
by eliminating nondeterminism with LR-based parsing methods.
The requirement that the program to be inverse should be expressed in functional language makes it impossible to apply it to complementary synthesis.}

\textbf{Srivastava et.al \cite{prog_inv_rev} assume that an inverse program is typically related to the original program,
...
Bounded model checking(BMC) \cite{bmc_tacas99} is a model checking technology that considers only limited length path.
...
Avnit et.al \cite{converter_date08,converter_todeas09} first defines a general model for describing the different protocols.
...
This paper proposes a faster and more direct halting algorithm that checks whether a particular encoder has corresponding decoder.
...
Experimental results shows that this paper's approach is much faster than that of \cite{ShengYuShen:fmcad10}.
...
in Proc. IEEE ICCAD, Nov. 1996, pp. 220每227.
...
vol. 155, no. 12,
pp. 1549每1561, Dec. 2007.

\bibitem{EXTSAT}
N. E\'en and N. S\"orensson.,
"Extensible SAT-solver,"
in Proc. Int. Conf.
SAT, May 2003, pp. 502每518.


%\bibitem{interp_cav03}
%K. L. McMillan, "Interpolation and SAT-based model checking," Proc. International Conference Computer Aided Verification (CAV '03), pp. 1-13, Jul. 2003.
%
%\bibitem{Craig}
%W. Craig, "Linear reasoning: A new form of the Herbrand-Gentzen theorem," J. Symbolic Logic, vol.22, no.3, pp. 250-268,1957.
...
Tech. J., vol. 34, pp. 1045每1079, Jan. 1955.
...
in Proc. IEEE FMCAD, Oct. 2000, pp. 108每125.


%
%\bibitem{buildcircuit_cav00}
%J. Kukula, and T. Shiple, "Building circuits from relations," Proc. International Conference Computer Aided Verification (CAV '00), pp. 113-123, jul. 2000.
%
%\bibitem{conssyn_dac03}
%J. Yuan, K. Albin, Adnan Aziz, and C. Pixley, "Constraint synthesis for environment modeling in functional verification," Proc. Design Automation Conference(DAC '03), pp. 296-299, Jun. 2003, doi:10.1145/775832.775909.
%
%\bibitem{modcons_iccad99}
%J. Yuan, K. Shultz, C. Pixley, H. Miller, and A. Aziz, "Modeling design constraints and biasing in simulation using BDDs," Proc. International Conference on Computer-Aided Design (ICCAD '99), pp. 584-590, Jun. 1999.
%
%\bibitem{borel_vlsi89}
%R. Brayton, and F. Somenzi, "Boolean relations and the incomplete specification of logic networks," Proc. International Conference on Computer-Aided Design (ICCAD '89), pp. 316-319, Nov, 1989.
%
%\bibitem{interpfun_iccad09}
%J. Jiang, H. Lin, and W. Hung, "Interpolating Functions from Large Boolean Relations," Proc. International Conference on Computer-Aided Design (ICCAD '09), pp. 779-784 , Oct. 2009, doi:10.1145/1687399.1687544.
%
%\bibitem{interp_huang95}
%G. Huang.
%Constructing Craig interpolation formulas.
%In Computing and Combinatorics,
%LNCS 959, pages 181-190, 1995.
%
%\bibitem{interp_kra97}
%J. Kraj\'i\~cek.
...
in Proc. ACM PPDP, Jul. 2010, pp. 13每24.
...
in Proc. IEEE DATE Conf. Exposit., Mar. 2008, pp. 294每299.
...
in Proc. IEEE DATE Conf. Exposit., Mar. 2009, pp. 129每134.
...
in Proc. Int. Conf. TACAS, Mar. 2010, pp. 343每348.
...
\textbf{First, I add the 1st paragraph in Section \ref{sec_intro} to provide background and motivation}.

\textbf{Second, I add the 2nd paragraph in subsection\ref{subsec_chkextdec} to explain why the protocols I consider are lossless}.
...
\textbf{Yes,
you are right.
I add the 5th, 6th and 7th paragraphs in Section \ref{sec_intro} to mention this point.}
...
\textbf{I add Subsections \ref{subsec_area} and \ref{subsec_timing} to compare the quality of decoders built manually and generated automatically.}
...
\textbf{I add the 4th paragraph in Subsection \ref{subsec_recdia} to explain the concept of loop-like path.}
...
\textbf{According to Definition \ref{def_pcc},
$M$ is loseless if its input can be uniquely determined by its output,
with some valuation of $p$, $d$ and $l$.
This definition does NOT need the concept of loop-like path.
}

\textbf{On the other hand,
the concept of loop-like path is used in Definition \ref{def_lnc},
which defines how to determine whether $M$ is loss.
I add the 3th paragraph in Subsection \ref{subsec_deterNo} to explain this idea.
Please also refer to the two paragraph before it.
}
...
\textbf{I add the last three paragraphs in Subsection \ref{subsec_chkextdec} to explain this.}
...
we try to infer assertion for complementary synthesis.
In that new algorithm,
this paper's algorithm are called hundreds of times,
and result in thousands of seconds of run time.
Should I mention this in my paper?
}
...
\textbf{Yes,
you are right.
I add the first paragraph in Section \ref{sec_intro} to handle this problem.}
...
We also explain why their approaches can't be applied to our problem.}
...
\textbf{Yes,
you are right.
Please refer to the first three paragraphs in Subsection \ref{subsec_deterNo}.}
...
Subsection \ref{subsec_proinv},
 and the last paragraphs in Subsection \ref{subsec_bmc_relate}.}
...
the subsection \ref{subsec_chkextdec} give enough details for them.
}
...
\textbf{But I think there are some litter confusions between us.}

\textbf{First, my previous TCAD paper actually describe two steps of
complementary synthesis, the first step is checking the existence of
decoder, while the second step build the decoder. The first step is a
very simple one, which only check the existence but not the
non-existence of decoder, this is the reason that make it never stop.
And most complexity of that paper is in its second step.}

\textbf{On the other hand, this new TCAD paper describe a totally new and
...
is just following the idea pop up in my mind, and publishing the results.}
